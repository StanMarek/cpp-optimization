...
solution lag(double a, double b, double epsilon, double gamma, int Nmax, matrix *ud, matrix *ad)
{
	...
	while (true)
	{
		...
		if (m <= 0) // <----- tego gowna nie bylo na schemacie blokowym ale chujnia chlopaki co robimy?
		{
			C.x = NAN;
			C.y = NAN;
			return C;
		...
		else if (C.x < D.x && C.x < B.x && D.x < B.x)
		{
			if (D.y < C.y)
			{
				A = C;
		...
		(*ud).add_row((B.x() - A.x()));
#endif
		if ( (B.x - A.x < epsilon) || (abs(D.x(1) - D.x(0) <= gamma)) )
			return C;
		D_old = D;
	}
}
